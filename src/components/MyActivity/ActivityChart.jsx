...
const calcDomain = (data, metricKey) => {
  ...
  // min == max 인 경우 살짝 벌려주기 (축 이상해지는 것 방지)
  if (minY === maxY) {
...
export default function ActivityChart({ bodyRecords, profile }) {
  ...
  const sortedData = useMemo(() => {
    if (!hasData) return [];
    return [...bodyRecords].sort((a, b) => (a.date > b.date ? 1 : -1));
  }, [bodyRecords, hasData]);
  ...
  // 최신 기록이 바뀌면 활성 기록도 같이 변경
  useEffect(() => {
    setActiveRecord(latestRecord);
  }, [latestRecord]);

  const currentRecord = activeRecord ?? latestRecord ?? null;
  ...
  // 표준/이상/이하 상태 (프로필 + 현재 선택 기록 기준)
  const metricStatuses = useMemo(() => {
    if (!profile || !currentRecord) {
      return {
        weight: "normal",
        muscle: "normal",
        fat: "normal",
    ...
    return {
      weight: getMetricStatus("weight", currentRecord, profile),
      muscle: getMetricStatus("muscle", currentRecord, profile),
      fat: getMetricStatus("fat", currentRecord, profile),
    };
  ...
  if (!hasData || !latestRecord) {
    return (
      <section className="w-full bg-sky-50 rounded-3xl border border-sky-100 p-4 sm:p-6 shadow-sm text-center text-sm text-slate-500">
        아직 체성분 기록이 없습니다. 아래에서 기록을 추가하면 그래프가
        나타납니다.
      </section>
    );
  }

  // 차트 클릭 시 해당 날짜의 기록을 활성화
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
          {Object.values(METRICS).map((m) => (
            <MetricCard
              key={m.key}
              label={m.label}
              value={formatNumber(currentRecord?.[m.key], 1)}
              active={selectedMetric === m.key}
              status={metricStatuses[m.key]}
